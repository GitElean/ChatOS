/* 
 * Server Component:
 * - Atencion con multithreading
 * - Broadcasting y mensajes directos
 * - Registro de usuarios
 * - Liberacion de usuarios
 * - Manejo de status
 * - Respuesta a solicitudes de informacion de usuario(s)
 * Autores: Elean Rivas, Javier Alvarez
*/
...
#define QUEUE_MAX 10 //numero maximo de conexiones en la cola
#define MAX_CONNECTIONS 20 //maximo de conexiones que el servidor soporta simultaneamente
#define MAX_BUFFER 8192 //buffer maximo por mensaje
#define INACTIVE_TIME 20 //tiempo para que el server coloce status inactivo

//map to mannage each clients time 
std::map<int, float> tiemposInactivos;

// Opciones de solicitudes el cliente puede hacerle al server
enum ClientOpt {
    SYNC = 1,
    CONNECTED_USERS = 2,
    STATUS = 3,
...
// Opciones de respuestas que el servidor puede dar al cliente
enum ServerOpt {
    BROADCAST_S = 1,
    MESSAGE = 2,
    ERROR = 3,
...
// Estrucauta de la cola para mandar mensajes
typedef struct {
...
// Estructuras para guardar las colas y clientes
typedef struct {
...
// Struct con toda la informacion relevante del cliente
struct Client
{
    int socketFd;
    std::string username;
...
/* 
 * Conexion y binding obtenido de: https://www.bogotobogo.com/cplusplus/sockets_server_client.php
*/
int main(int argc, char *argv[])
{
    ...
    std::cout << MAGENTA <<"**** Si desea salir, ingresar 'exit' ****" << DEFAULT << std::endl;
    ...
    std::cout << "Terminando las conexiones... " << std::endl;
...
/*
 * Metodo para mandar un error al cliente cuando no se envia informacion
 * a la solicitud o a otro trouble handler
...
/*
 * Thread para escuchar nuevas conexiones de clientes al servidor
 * cuando termine de aceptar y cree una nueva conexion y un nuevo socket,  
 * se queda escuchando 
*/
void *listen_to_connections(void *params){
...
/*
 * bind() unira el socket a la direccion IP actual en el puerto
*/
void bind_socket(struct sockaddr_in serverAddr, int socketFd, long port){
...
/*
 * Metodo para insertar y crear un nuevo cliente en el chat
*/
void new_client(chat_data *chat, int new_socket){
...
/*
 * Thread para escuchar solicitudes del cliente
 * por cada opcion en las ClientOpt un conjunto de 
...
/*
 * Metodo para encontrar un nombre de usuario dado un id
*/
...
/*
 * Metodo que toma el tiempo cuando el usuario esta inactivo
*/
void *timer(void *params) {
